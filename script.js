...
const columnCounter = document.querySelector('#columnCounter');
const rowCounter = document.querySelector('#rowCounter');
const columnSlider = document.querySelector('#columnSlider');
const rowSlider = document.querySelector('#rowSlider');
const invencibility = document.querySelector('#invencibility');
...
developerMenuButton.addEventListener('click', toggleDevMenu);

rowSlider.addEventListener('input', () => {
	rowCounter.innerText = rowSlider.value;
	grid.rows = rowSlider.value;
});

columnSlider.addEventListener('input', () => {
	columnCounter.innerText = columnSlider.value;
	grid.columns = columnSlider.value;
});

bonusButton.addEventListener('click', () => {
	if (bonusIcon.innerHTML == '<i class="fa-regular fa-star"></i>') {
		bonusIcon.innerHTML = '<i class="fa-solid fa-star"></i>';
		grid.bonus = true;
	} else {
		bonusIcon.innerHTML = '<i class="fa-regular fa-star"></i>';
		grid.bonus = false;
	}
});

invencibility.addEventListener('click', () => {
	if (
		invencibility.innerHTML == '<i class="fa-regular fa-circle-check"></i>'
	) {
		invencibility.innerHTML = '<i class="fa-regular fa-circle"></i>';
		snake.dies = false;
	} else {
		invencibility.innerHTML = '<i class="fa-regular fa-circle-check"></i>';
		snake.dies = true;
...
function toggleMusic() {
	if (musicFile.paused) {
		musicFile.play();
		musicButton.innerHTML = '<i class="fa-solid fa-volume"></i>';
	} else {
		musicFile.pause();
...
function makeGrid() {
	...
	if (walls == true) {
		const firstRowPixels = document.querySelectorAll('.r1.pixel');
		firstRowPixels.forEach((pixel) => {
			pixel.style.borderTop = '5px solid #2c2c2c';
		});

		const lastRowPixels = document.querySelectorAll(`.r${grid.rows}.pixel`);
		lastRowPixels.forEach((pixel) => {
			pixel.style.borderBottom = '5px solid #2c2c2c';
		});

		const firstColumnPixels = document.querySelectorAll('.c1.pixel');
		firstColumnPixels.forEach((pixel) => {
			pixel.style.borderLeft = '5px solid #2c2c2c';
		});

		const lastColumnPixels = document.querySelectorAll(
			`.c${grid.columns}.pixel`,
		);
		lastColumnPixels.forEach((pixel) => {
			pixel.style.borderRight = '5px solid #2c2c2c';
		});
	}
	bonus.positions = [
		document.querySelector('.r1.c1'),
		document.querySelector(`.r1.c${grid.columns}`),
		document.querySelector(`.r${grid.rows}.c1`),
		document.querySelector(`.r${grid.rows}.c${grid.columns}`),
	];
}
...
function returnToMenu() {
	...
	developerMenuButton.style.display = 'flex';
...
async function startGame(caller) {
	...
	players[currentPlayer].difficulties[players[currentPlayer].turn] =
		difficulty;
	developerMenuButton.style.display = 'none';
...
function beat() {
	if (isPaused == false) {
		...
		players[currentPlayer].bestTurn = players[
			currentPlayer
		].maxScores.indexOf(Math.max(...players[currentPlayer].maxScores));
		if (grid.isFull == false) {
			fullGrid();
		}
		checkColision();
		drawSnake();
...
function startSnake() {
	//safe 3-pixel margin
	snake.row = Math.floor(Math.random() * (grid.rows - 6) + 4);
	snake.column = Math.floor(Math.random() * (grid.columns - 6) + 4);
...
function overflow() {
	if (!walls) {
		if (snake.row > grid.rows && snake.direction == 'down') {
			snake.row = 1;
		}
		if (snake.row < 1 && snake.direction == 'up') {
			snake.row = grid.rows;
		}
		if (snake.column > grid.columns && snake.direction == 'right') {
			snake.column = 1;
		}
		if (snake.column < 1 && snake.direction == 'left') {
			snake.column = grid.columns;
		}
	} else {
		if (snake.row > grid.rows && snake.direction == 'down') {
			snake.row = 1;
...
//generate and draw apple only where there is no snake body or bonus
function generateApple() {
	bonus.wasGenerated = false;
	apple.count++;
	do {
		apple.row = Math.floor(Math.random() * (grid.rows - 1) + 1);
		apple.column = Math.floor(Math.random() * (grid.columns - 1) + 1);
...
function incrementScore(growth) {
	...
	if (parseInt(maxScore.innerText) < parseInt(scoreboards[0].innerText)) {
		...
		players[currentPlayer].maxScores[players[currentPlayer].turn] =
			parseInt(scoreboards[0].innerText);
	}
}
...
//full grid bonus
function fullGrid() {
	if (snake.allPositions.length == grid.rows * grid.columns) {
		grid.isFull = true;
		incrementScore(100);
	}
}
...
//player wins if the snake has filled the whole grid
// (score == rows * columns - 2 points, as the snakes starts with length 2)
// if the grid was filled, the player shold have 100 more points
function win() {
	if (
		(players[currentPlayer].scores[players[currentPlayer].turn] >=
			grid.columns * grid.rows - 2 &&
			grid.isFull == false) ||
		(players[currentPlayer].scores[players[currentPlayer].turn] >=
			grid.columns * grid.rows + 98 &&
			grid.isFull == true)
	) {
...
function toggleDevMenu() {
	if (
		...
		developerMenu.style.display == ''
	) {
		developerMenu.style.display = 'flex';
		developerMenuButton.style.borderRadius = '0 0 10px 10px';
	} else {
		developerMenu.style.display = 'none';
		developerMenuButton.style.borderRadius = '10px 10px 10px 10px';
	}
}
